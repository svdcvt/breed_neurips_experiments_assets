...
from melissa.server.parameters import StaticExperiment
...
class ICSamplerMixIn:
    ...
    def sample(self, nb_samples=1):
        amp = jax.random.uniform(self.amp_key, (nb_samples,), minval=self.l_bounds[0], maxval=self.u_bounds[0])
        phs = jax.random.uniform(self.phs_key, (nb_samples,), minval=self.l_bounds[1], maxval=self.u_bounds[1])
        
        return np.asarray(jnp.stack((amp, phs), axis=1).squeeze()).astype(self.dtype)      

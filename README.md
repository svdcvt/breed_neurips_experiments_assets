...
However, to respect double-blind review process, we have to anonymise everything.

We fully anonymise the codebase itself, i.e., the algorithm code, scripts that create all configuration files, scripts that run the experiments, scripts that process the results of the experiments and create the plots that are included in the paper.
...
However, we have to anonymise it too, i.e., exclude any paths, usernames, mention of clusters/supercomputers names that can reveal our identities or/and affiliation. 
...
|---        |---|---|---|
| **Code** | <ul><li>Algorithms codes</li><li>Scripts "end-to-end": <ul><li>generating config. files</li><li>running experiments</li><li>processing exp. results</li><li>creating plots</li></ul></li><li>Part of result files:<ul> <li>combined `.csv` A, B</li></ul></li></ul>  | <ul><li>Table A statistics <ul><li>for checkpoints 1k, 2k, **3k**, 4k</li></ul></li><li>Figure 3<ul><li>for all PDE cases</li><li>**rollout** and other metrics from Table A</li></ul></li><li>Figure 4</li></ul> Can produce the same configs used for ExpA and ExpB.| **This repository.** <br> Code: <ul><li>[Common](./common/README.md)</li><li>[Scripts](./scripts/README.md)</li></ul> Data and plotting: <ul><li>Main experiments (ExpA): [combined `.csv`](./validation_results/validation_results_decay/combined_validation_results.csv)</li><li>Random seed experiments (ExpB): [combined `.csv`](./validation_results/random_seed_tensorboard_results.csv)</li><li>Plotting these results: [ipynb](./validation_results/csv_to_tab_plot.ipynb)</li></ul>  Fork of Melissa with active sampling: [url](url |
...
|**+models**  |  For ExpA, per PDE, per method: <ul><li>model weights chkpt every 1k iterations</li></ul> | <ul><li>`.npy` data for plots</li><li>`.csv` metrics</li><li>`.pdf` plots</li></ul> | Download [here](url and unzip[^1] into `experiments` directory, can be combined with level "+tensorboard"|

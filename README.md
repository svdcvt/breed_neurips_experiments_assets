...
|---        |---|---|---|
| **Code**<br>**+configs** | <ul><li>Algorithms codes</li><li>Experiments codes</li><li>Scripts "end-to-end": <ul><li>generating config. files</li><li>running experiments</li><li>processing exp. results</li><li>creating plots</li></ul></li><li>Config. files for all experiments</li><li>Part of results files:<ul> <li>combined `.csv` A, B</li></ul></li></ul>  | <ul><li>Table A statistics <ul><li>for checkpoints 1k, 2k, **3k**, 4k</li></ul></li><li>Figure 3<ul><li>for all PDE cases</li><li>**rollout** and other metrics from Table A</li></ul></li><li>Figure 4</li></ul> Can produce the same configs used for ExpA and ExpB, and to create validation datasets.| **This repository.** <br> Code: <ul><li>[Common](./common/README.md)</li><li>[Scripts](./scripts/README.md)</li></ul> Data and plotting: <ul><li>Main experiments (ExpA): [combined `.csv`](./validation_results/validation_results_decay/combined_validation_results.csv)</li><li>Random seed experiments (ExpB): [combined `.csv`](./validation_results/random_seed_tensorboard_results.csv)</li><li>Plotting these results: [ipynb](./validation_results/csv_to_tab_plot.ipynb)</li></ul>  Fork of Melissa with the algorithm: <br>[here](https://zenodo.org/records/15490672/files/melissa.zip?download=1&preview=1), instructions [here](#installation-instructions)<br> Environment container:<br> download [here](https://zenodo.org/records/15490672/files/melissa-active-sampling-with-apebench-cuda.sif?download=1&preview=1) (~6Gb), instructions [here](#container-usage-instructions) |
|**+data**  |  For ExpA, per PDE, per method, per model checkpoint: <ul><li>`.npy` data for plots</li><li>`.csv` metrics</li><li>`.pdf` plots</li></ul> |  Combined `.csv` , and for any PDE/method, available chkpt:<ul><li>Figure 1ab</li><li>Figure 2</li></ul>   | Download [here](https://zenodo.org/records/15490672/files/validation_results.zip?download=1&preview=1) (~1.5GB) and unzip into `validation_results` directory |
|**+models, +valset**  |  For ExpA, per PDE, per method: <ul><li>model weights chkpt every 1k iterations</li></ul> Full validation datasets for all PDEs| <ul><li>`.npy` data for plots</li><li>`.csv` metrics</li><li>`.pdf` plots</li></ul> | Download chkpts [here](https://zenodo.org/records/15490672/files/set_models.zip?download=1&preview=1) (~3Gb) and unzip[^2] into `experiments` directory, can be combined with level "+tensorboard". <br> Download val. datasets [here](https://zenodo.org/records/15490672/files/validation_results.zip?download=1&preview=1) (~15GB). |
|**+tensorboard**  |  For ExpB, per PDE, per method:<ul><li>`tensorboard` event file</li></ul> | <ul><li> ExpB combined `.csv` B</li><li> can see all other statistics, e.g., training loss  | Download [here](https://zenodo.org/records/15490672/files/random_seed_tb.zip?download=1&preview=1) (~2.5Gb), unzip[^2] into `experiments` directory, can be combined with level "+models" |

Download-all link: https://zenodo.org/api/records/15490672/draft/files-archive (~28Gb)
...
2*: It has to be unzipped the right way so to preserve directories structure.
...
We provide a Singularity container, to which all the executable scripts are referring to. It was used for all the experiments. The container can be downloaded from here: [here](https://zenodo.org/records/15490672/files/melissa-active-sampling-with-apebench-cuda.sif?download=1&preview=1) (~6Gb).
The container is expected (by the scripts) to be located in the root of the repository. 

You can enter the container shell easily with the following command:
```bash
...
We provide the instructions to install dependencies without the container, but we do not guarantee the exact environment reproducibility or an easy installation process.
...
```bash
python3 scripts/utils/hello.py
```
...
unzip <path/to/melissa.zip> -d $HOME/melissa

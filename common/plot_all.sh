...
for dir in $subdirs; do
    ...
    python3 plot_model_predictions.py --study-paths $pathhs --model-id 1000 --all-plots --output-dir ../validation_results_decay/
    echo "<<<<<< Model 2000"
    echo "Current time: $(date +%T)"
    python3 plot_model_predictions.py --study-paths $pathhs --model-id 2000 --all-plots --output-dir ../validation_results_decay/
    echo "<<<<<< Model 3000"
    echo "Current time: $(date +%T)"
    python3 plot_model_predictions.py --study-paths $pathhs --model-id 3000 --all-plots --output-dir ../validation_results_decay/
    echo "<<<<<< Model 4000"
    echo "Current time: $(date +%T)"
    python3 plot_model_predictions.py --study-paths $pathhs --model-id 4000 --all-plots --output-dir ../validation_results_decay/
done
# will time overall how long it took to run
end_time=$(date +%s)

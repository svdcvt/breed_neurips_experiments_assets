...
if __name__ == "__main__":
    parse = argparse.ArgumentParser(description="Find validation IDs and save all in csv")
    parse.add_argument(
        "--subdir-match",
        default="diff_*",
        help="Subdirectory to match in $DATASET_ROOT",
    )
    parse.add_argument(
        "--num",
        type=int,
    ...
    if len(all_files) == 0:
        raise FileNotFoundError(f"Files not found with {validation_file_path}")

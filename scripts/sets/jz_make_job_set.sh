...
config_files=($(find "$abs_path" -type f -name "config*slurm.json" ! -name "*offline*"))
...
for config_file in "${config_files[@]}"; do
	...
	echo "srun singularity exec --env APEBENCH_ROOT=\"\$WORK/apebench/test\" \${singularity_container} melissa-launcher --config_name $config_path" >> "$script_name"
done

# Make the script executable
chmod +x "$script_name"
echo "Script $script_name created successfully."
echo "You can run it with: sbatch $script_name"

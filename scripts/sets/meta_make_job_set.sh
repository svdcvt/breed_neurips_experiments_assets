...
# than gather all the scripts names in a file that will do CLUSTER_SUBMIT script_name
# scripts made by make_job_set.sh are named job_set_0.sh inside the folder
# and the script that will do CLUSTER_SUBMIT is named job_set_meta_{filter}.sh
...
for folder in "${filtered_directories[@]}"; do
    ...
    if [ -f "$script_name_" ]; then
        if [ -z "$flag_delete" ]; then
            echo "CLUSTER_SUBMIT $script_name_" >> "$script_name"
        else
